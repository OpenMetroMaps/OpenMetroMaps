...
public class TestImportOverpassDuesseldorf
{
			...
			IOException, ParserConfigurationException, TransformerException
	{
		...
		ModelData data = importer.execute(OverpassQueries.Q_RHEINBAHN,
				new RouteTypeFilter("train", "subway", "tram"), prefixes,
				suffixes, fixes);

		System.out.println(String.format("Imported %d lines with %d stations",
				data.lines.size(), data.stations.size()));

		Path pathOutput = Paths.get("/tmp/rheinbahn.omm");
